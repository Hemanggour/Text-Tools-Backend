...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            voice = (
                input("Enter voice (optional, press Enter to skip): ").strip() or None
            )
            emotion = (
                input("Enter emotion (optional, press Enter to skip): ").strip() or None
            )

            response = test_file.text_to_speech_api(
                api_key=api_key, query=text, voice=voice, emotion=emotion
            )
